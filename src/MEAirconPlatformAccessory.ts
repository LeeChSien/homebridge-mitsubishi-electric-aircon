...
export class MEAirconPlatformAccessory extends MEAircon {
  ...
  async init() {
    ...
    const uuid = this.platform.api.hap.uuid.generate(
      this.identity.serial || `IP_${this.configs.ip}`,
    )
    ...
    this.service
      ...
      .onGet(() => {
        ...
        if (
          driveMode === DRIVE_MODE.AUTO_COOLER ||
          driveMode === DRIVE_MODE.COOLER ||
          driveMode === DRIVE_MODE.DEHUM
        ) {
          if (
            this.getAttribute('roomTemperature') <=
        ...
        if (
          driveMode === DRIVE_MODE.AUTO_HEATER ||
          driveMode === DRIVE_MODE.HEATER
        ) {
          if (
            this.getAttribute('roomTemperature') >=
            this.getAttribute('temperature')
    ...
    this.service
      ...
      .setProps({ minValue: 1, maxValue: 100, minStep: 25 })
      .onSet((value) => {
        const level = Math.ceil((value as number) / 25)
        if (level === 4) {
          this.setWindSpeed(5)
        } else {
          this.setWindSpeed(level)
        }
      })
      .onGet(() => {
        const windSpeed = this.getAttribute('windSpeed')
        if (windSpeed === 5) {
          return 100
        } else {
          return windSpeed * 25
    ...
    this.service
      ...
      .onGet(() => this.getAttribute('swingMode'))
  }
}
